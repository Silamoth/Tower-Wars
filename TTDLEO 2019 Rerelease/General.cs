...
// Type: TTDLEO_2019_Rerelease.General
...
{
  internal class General : SupportSoldier
  {
    public General(ContentManager content, Vector2 target, Vector2 position)
    {
      this.Position = position;
      this.Target = target;
      this.LoadContent(content);
      this.LoadSprite(this.Texture, content);
      this.sprite.AttackInterval += 140f;
      this.Stat = 1;
      this.Speed = 1f;
      this.Health = 3;
      this.ActTime = 250;
      this.Reward = 3;
      this.Name = nameof (General);
    }
    ...
    {
      this.Texture = content.Load<Texture2D>("generalSpritesheet");
    }
  }
}
